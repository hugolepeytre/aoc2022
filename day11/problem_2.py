...
def simulate_cycle(inventories, activities, instructions, lcm):
    for i in range(len(inventories)):
        activities[i] += len(inventories[i])
        while len(inventories[i]) > 0:
            item = inventories[i].pop(0)
            item = instructions[i][0](item) % lcm
            target = (
                instructions[i][2]
...
with open("day11/input") as file:
    ...
    lcm = 1
    for _, div, _, _ in monkey_instructions:
        lcm *= div
    print(lcm)
    for i in range(10000):
        monkey_inventories, monkey_activities = simulate_cycle(
            monkey_inventories, monkey_activities, monkey_instructions, lcm
        )
