...
def add_sand(grid, grain_x, grain_y, min_x):
    grain_y = sand_straight_fall(grid, grain_x, grain_y)
    if grain_y == len(grid[0]) - 1:
        grid[grain_x, grain_y] = 1
        return grid, min_x
    if grain_x == 0:
        new_col = np.zeros(len(grid[0]))
        new_col[-1] = 1
        grid = np.vstack((new_col, grid))
        return grid, min_x - 1
    if grid[grain_x - 1][grain_y + 1] == 0:
        return add_sand(grid, grain_x - 1, grain_y + 1, min_x)
    if grain_x == len(grid) - 1:
        new_col = np.zeros(len(grid[0]))
        new_col[-1] = 1
        grid = np.vstack((grid, new_col))
        return grid, min_x
    if grid[grain_x + 1][grain_y + 1] == 0:
        return add_sand(grid, grain_x + 1, grain_y + 1, min_x)
    grid[grain_x, grain_y] = 1
...
with open("day14/input") as file:
    ...
    grid, min_x = add_sand(grid, origin_x, 0, min_x)
    while grid[origin_x][0] == 0:
        grid, min_x = add_sand(grid, origin_x, 0, min_x)
        sand_grains += 1
        origin_x = 500 - min_x
    print(sand_grains)
