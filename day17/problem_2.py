...
with open("day17/input") as file:
    # After exploratory analysis, first 1757 moves getting to a length of 2722
    # Then cycles have size 1755 and increase the height by 2747
    start = time.time()
    gas_shot = [0]
    gas = file.read().strip()
    tunnel = [[1] * 7]
    first_cycle_length = 1757
    first_cycle_height = 2722
    cycle_length = 1755
    cycle_height = 2747
    steps_after_first_cycle = 1000000000000 - first_cycle_length
    for i in range(first_cycle_length + (steps_after_first_cycle % cycle_length)):
        simulate_fall(tunnel, gas, i % 5)
    print(len(tunnel) - 1 + ((steps_after_first_cycle // cycle_length) * cycle_height))
