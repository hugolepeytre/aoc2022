...
with open("day12/smol_input") as file:
    grid = []
    start = None
    end = None
    ...
    while len(marked) > 0:
        ...
        if grid[current_y][current_x] == 97:
            print(distance)
            exit()
        if not (current_x, current_y) in visited:
            current_val = grid[current_y][current_x]
            if current_x > 0 and grid[current_y][current_x - 1] >= current_val - 1:
                marked.append(((current_x - 1, current_y), distance + 1))
            if current_y > 0 and grid[current_y - 1][current_x] >= current_val - 1:
                marked.append(((current_x, current_y - 1), distance + 1))
            if (
                current_x < len(grid[0]) - 1
                and grid[current_y][current_x + 1] >= current_val - 1
            ):
                marked.append(((current_x + 1, current_y), distance + 1))
            if (
                current_y < len(grid) - 1
                and grid[current_y + 1][current_x] >= current_val - 1
            ):
                marked.append(((current_x, current_y + 1), distance + 1))
