...
def solve(graph, flows, known, current=0, opened=0, step=4):
    if step > 29 or opened == (1 << 15) - 1:
        return 0
    sols = []
...
with open("day16/input") as file:
    ...
    known = dict()
    best = 0
    full_opened = (2 << len(graph)) - 1
    for i in range((2 << len(graph) - 1) - 1):
        best = max(
            best,
            solve(graph, flows, known, opened=i)
            + solve(graph, flows, known, opened=full_opened - i),
        )
    print(best)
    print(int(time.time() - start))

# TODO : Run for the agent separately with every possible valve limitation combination (2^15 runs) then sums complementary limitations and take the max
# TODO : Add heuristic check to stop a run early if it has no chance of beating max ? (check improvements on part 1 first : stop path if all valves open)
