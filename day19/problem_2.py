...
def quality_rec(
    ...
    if current_minute >= N_MINUTES:
        return 0
    remaining_building_steps = N_MINUTES - 1 - current_minute
    ...
    for i, (cost, new_r) in enumerate(zip(costs, new_robots)):
        ...
        added_score = N_MINUTES - robot_finish_time if sum(new_r) == 0 else 0
        new_resources = fix_to_max(new_resources)
        if negative_index is None and robot_finish_time <= N_MINUTES:
            sols.append(
                added_score
                + quality_rec(
                    dynamic,
...
with open("day19/input") as file:
    start = time.time()
    qualities = [quality(*numbers(blueprint)) for blueprint in file.readlines()[:3]]
    print(qualities)
    print(sum(qualities))
